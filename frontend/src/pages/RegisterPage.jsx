...
export default function RegisterPage() {
  const [form, setForm]   = useState({ username: '', email: '', password: '', role: 'student', department: '' })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0e1a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#131929', border: '1px solid #1e2d45', borderRadius: '16px', padding: '40px', width: '100%', maxWidth: '420px' }}>
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          <div style={{ fontSize: '36px', marginBottom: '10px' }}>📝</div>
          <h1 style={{ color: '#e2e8f0', fontSize: '20px', fontWeight: '800', margin: 0 }}>Create Account</h1>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          {[['Username', 'username', 'text'], ['Email', 'email', 'email'], ['Password', 'password', 'password']].map(([label, field, type]) => (
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <input type="text" value={form.department}
              ...
              style={inputStyle} placeholder="e.g. Information Technology" />
          </div>

          <button type="submit" disabled={loading} style={{
            width: '100%', padding: '12px', borderRadius: '8px',
            background: loading ? '#1e2d45' : '#10b981', color: 'white',
